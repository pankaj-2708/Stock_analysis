...
def getCompName():
    ...
    return soup.find(class_="margin-0 show-from-tablet-landscape").text.replace(
        " Ltd", ""
    )[:16]
...
def getKeymetrics(company_name="BANKINDIA"):
    ...
    metrics_value = [
        " ".join(i.text.replace("\n", "").replace("â‚¹", "").strip().split())
        for i in metrics_value
    ]

    collecthtml(
        f"https://www.alphaspread.com/security/nse/{company_name.lower()}/dcf-valuation/base-case"
    )
    ...
    try:
        metrics_value.append(
            round(
                float(metrics_value[1].replace(",", ""))
                / float(metrics_value[4].replace(",", "")),
                2,
            )
        )
    ...
    metrics_value = [
        "None" if type(i) == str and len(i) == 0 else i for i in metrics_value
    ]
...
def peers_table(company="BANKINDIA"):
    ...
    for rows in ind_columns:
        ...
        for i in range(len(clm)):
            clm[i] = (
                float(clm[i].replace(",", ""))
                if clm[i].replace(",", "").replace(".", "").replace("-", "").isdigit()
                else None
            )
